...
class Lifecycle:
    ...
    def _init_tts(self) -> None:
        if self.config.tts.engine == "elevenlabs" and self.config.tts.elevenlabs_api_key:
    ...
    async def speak(self, text: str, emotion: str) -> str:
        pose = EMOTION_POSE_MAP.get(emotion, "speaking")
        self.state.set_many(pose="speaking", emotion=emotion, is_speaking=True)

        path = await self._tts.synthesize(text, emotion)

        def on_done():
            self.state.set_many(pose=pose if pose != "speaking" else "idle", is_speaking=False)
